...
def init_choose_room(required_room_list, optional_room_list):
    ...
    if (room_type_choice <= 0.4 and required_room_list) or not optional_room_list:  # Here the float is used as a
        ...
        chosen_room = required_room_list.pop(
            0)  # this removes the room from the required_room_list and selects it to be returned at the end of the
        # function, Rooms are picked in order to avoid the player being trapped
        ...
        chosen_room = optional_room_list.pop(random.randint(0,
                                                            len(optional_room_list) - 1))  # Here the rooms are
        # picked in a random order as the order is not important, and is selected to be returned
...
def init_find_exit(branch_from_room, possible_exits):
    """
    This function takes the branch room and the list of possible __exits as arguments and returns a string of the
...
def init_gen_empty_map_matrix(map_size=[5, 5]):
    """
    This function takes a list of 2 integers as an arguments (size of the matrix wanted) and generates a matrix of
    the size specified in the argument (or the base case [5, 5]), The matrix is initialised with only None values,
    and It returns the matrix at the end of the function
...
class Map:
    ...
    Any functions built into this class that begin with 'init' are labeled because they are used for the
    initialisation of the map

    The map is generated randomly by having a starting point (entrance at the center of the matrix), Then the next
    room to be added is picked at random from the 2 room lists, The rooms in the optional_room_list are picked in a
    random order as it is not import where they are accessed, The rooms in the required_room_list are picked in the
    order of the list, this is to prevent situations where the player could get stuck behind a locked door with the
    key inaccessible behind the door, Next a random room is picked from the rooms already placed in the map (repeated
    until a room is found with an exit not already taken and not out of bounds)-This room is often called the
    branch_from_room in the code, Next an exit is picked from the possible __exits, After this the new room is added
    to the map and the correct __exits and lists are updated, This is repeated until both the room lists are empty
    """
    ...
    def __init__(self):
        ...
        """
        self.matrix_size = [5,
                            5]  # sets the size of the map matrix, the integers in the list need to be odd to have a
        # center for the entrance room
        self.rooms_objects = []  # has all the room objects in the map in a list
    ...
    def init_gen_map(self, required_room_list, optional_room_list):
        ...
        """
        # Here it initializes the entrance (first room), so the map creation can work
        ...
        while required_room_list or optional_room_list:  # runs a while loop until both lists are empty
            ...
            self.place_room(chosen_room, branch_from_room,
                            exits)  # This function handles the system for adding the room to the game matrix and
            # setting the correct __exits in the __exits dictionary of the room
    ...
    def init_check_exits_possible(self, branch_from_room):
        ...
        if branch_from_room.get_y() != 0 and self.map_matrix[branch_from_room.get_y() - 1][
            branch_from_room.get_x()] is None:
            north_possible = True
        # check east
        if branch_from_room.get_x() != self.matrix_size[0] - 1 and self.map_matrix[branch_from_room.get_y()][
            branch_from_room.get_x() + 1] is None:
            east_possible = True
        # check south
        if branch_from_room.get_y() != self.matrix_size[1] - 1 and self.map_matrix[branch_from_room.get_y() + 1][
            branch_from_room.get_x()] is None:
            south_possible = True
            # check west
        if branch_from_room.get_x() != 0 and self.map_matrix[branch_from_room.get_y()][
            branch_from_room.get_x() - 1] is None:
            west_possible = True
    ...
    def init_find_branch_room(self):
        ...
        while not found_branch_room:
            temp_index = random.randint(0,
                                        len(self.rooms_objects) - 1)  # here it selects a random integer for
            ...
            possible_exits = self.init_check_exits_possible(
                branch_from_room)  # uses function that returns a list of booleans of possible directions the room
            # can be placed in
            if True in possible_exits:
                return branch_from_room, init_find_exit(branch_from_room,
                                                        possible_exits)  # returns only if a room with a free exit
                # has been found

    def place_room(self, chosen_room, branch_from_room, current_exit):
        """
        This function takes the room being added to the map, the room it is being branched from, and the exit from
        the branched room that will lead to the new room as arguments, The function sets the room within the matrix
        and sets the __exits for both the rooms
        ...
        if current_exit == "north":  # This sets the exit for the chosen_room and sets the x,y and places the room
            # within the map matrix
    ...
    def display_map(self, player):
        """
        This function is used for displaying the map on a grid, it takes in the player object. it uses a for loop
        that creates 5 lines to be printed, once the first for loop has completed, the whole map should be printed
        This may later be changed slightly to incorporate where the player is, and only display rooms that have been
        visited
        ...
        *This shows room with no __exits
        ...
        for i in range(len(self.map_matrix)):
            ...
            for a in range(len(self.map_matrix[i])):
                ...
                if not room or not room.get_visited():  # Here it prints empty space if no rooms are found in
                    # that position of the matrix
                    line_1 += " " * 10
                    line_2 += " " * 10
                    line_3 += " " * 10
                    line_4 += " " * 10
