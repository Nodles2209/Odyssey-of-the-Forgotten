...
from rooms import Room 	# imports the claass used for creating room objects and functions for setting and getting variables
from player import Player 	#imports the player class used for holding data on the player

def check_win():
	'''This function will return True or False on whether the player has completed the game or not. The function itself will hold
	   The conditions on how the player can win'''
	return False
...
def normalise_input(player_action):
	...
	input_list = player_action.split(" ")	#This splits the players input into a list, by using a space character to split by
...
def execute_action(game_map, player, player_action):
	'''This function takes in the game_map object, the player object and the input from the player after it has been normalised.
	   The function expects the first word to be the action eg "go" and the second word to be things like direction eg "North"'''
